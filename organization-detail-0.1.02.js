(()=>{
  ...
  const SECTIONS=[
    {key:'neigongs',title:'Нейгуны',singular:'нейгун',add:'Добавить нейгун'},
    {key:'specialEffects',title:'Специальные эффекты',singular:'специальный эффект',add:'Добавить эффект'},
    {key:'specialSkills',title:'Специальные навыки',singular:'специальный навык',add:'Добавить навык'},
    {key:'techniques',title:'Приёмы',singular:'приём',add:'Добавить приём'},
    {key:'reworks',title:'Реворки приёмов',singular:'реворк',add:'Добавить реворк'}
  ];
  let all=D.loadAll();
  ...
  function renderEntry(key,row,index){
    const img=safeSrc(row.image),body=cleanRich(row.descriptionHtml||'');
    return `<article class="org-content-entry" data-entry="${esc(row.id)}">
      ...
      ${img?`<div class="org-content-image"><img src="${esc(img)}" alt="${esc(row.title||'')}"></div>`:''}
      ${body?`<div class="org-content-body rich-output">${body}</div>`:`<div class="org-content-body org-content-empty-text">Описание не добавлено.</div>`}
    </article>`;
  }
  ...
  function render(){
    ...
    const icon=safeSrc(item.thumbImage||item.image||''),hero=safeSrc(item.image||''),sections=content();
  ...
  async function saveEditor(){
    ...
    const row=D.normalizeDetailItem({id:editingItemId||undefined,title,image:pendingImage,descriptionHtml:cleanRich($('dRich').innerHTML)});
