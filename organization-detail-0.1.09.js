(()=>{
  ...
  function renderEntry(key,row,index){
    ...
    return `<article class="org-content-entry" data-entry="${esc(row.id)}">
      ...
      <div class="org-content-entry-main ${img?'':'no-image'}">${img?`<div class="org-content-image"><img src="${esc(img)}" alt="${esc(row.title||'')}"></div>`:''}${body?`<div class="org-content-body rich-output">${body}</div>`:`<div class="org-content-body org-content-empty-text">Описание не добавлено.</div>`}</div>
    </article>`;
  }
