(function(){
  if(window.__KF_REACTIONS_V0048__)return;
  window.__KF_REACTIONS_V0048__=true;

  const smileFile=n=>`smiles/smile_${String(n).padStart(3,'0')}.gif`;
  ...
  async function removeReaction(contentKey,entryId){
    const user=await getUser();
    if(!user)return;
    const {error}=await KFSupabase.client.from('content_reactions')
      .delete()
      .eq('content_key',contentKey)
      .eq('entry_id',String(entryId))
      .eq('user_id',user.id);
  ...
  function group(rows){
    const m=new Map();
    rows.forEach(r=>m.set(r.reaction,(m.get(r.reaction)||0)+1));
  ...
  async function mountSmiles(containerId,contentKey,entryId){
    ...
    el.classList.add('kf-reactions');
    el.innerHTML='<span class="kf-reaction-muted">Загрузка реакций...</span>';
    try{
      ...
      el.querySelector('.kf-reaction-add').onclick=()=>{
        ...
        p.querySelectorAll('[data-smile]').forEach(btn=>{
          btn.onclick=async()=>{
            const reaction='smile:'+btn.dataset.smile;p.classList.remove('open');
            try{
              if(my&&my.reaction===reaction)await removeReaction(contentKey,entryId);
              else await setReaction(contentKey,entryId,reaction);
              await mountSmiles(containerId,contentKey,entryId);
            }catch(e){alert(e.message||'Не удалось сохранить реакцию.')}
          };
        });
      };
    }catch{
      el.innerHTML='<span class="kf-reaction-muted">Реакции временно недоступны.</span>';
    }
  }
  ...
  async function mountCheck(containerId,contentKey,entryId){
    ...
    try{
      ...
      el.querySelector('button').onclick=async()=>{
        if(!user){alert('Чтобы поставить реакцию, войдите в аккаунт.');return}
        try{
          if(my)await removeReaction(contentKey,entryId);else await setReaction(contentKey,entryId,'check');
          await mountCheck(containerId,contentKey,entryId);
        }catch(e){alert(e.message||'Не удалось сохранить реакцию.')}
      };
    }catch{el.innerHTML=''}
  }
