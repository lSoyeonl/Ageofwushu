(()=>{
  ...
  const SECTIONS=[
    ...
    {key:'important',title:'Важное',add:'Добавить материал',singular:'материал',important:true}
  ];
  ...
  function renderSection(meta,arr){
    ...
    return `<section class="org-panel org-content-section ${meta.important?'org-important-section':''}" id="section-${esc(meta.key)}">
      ...
      <div class="org-content-stack">${arr.length?arr.map((row,i)=>renderEntry(meta.key,row,i)).join(''):`<div class="org-section-empty">${meta.important?'Важных материалов пока нет.':'Пока ничего не добавлено.'}${staff?' Используйте кнопку добавления выше.':''}</div>`}</div>
    </section>`;
  }
  ...
  function render(){
    ...
    $('detailApp').innerHTML=`<div class="detail-breadcrumb"><a href="schools.html?type=${encodeURIComponent(item.category)}&align=${encodeURIComponent(item.alignment)}">Школы, секты и фракции</a><span>›</span><span>${esc(item.name)}</span></div>
      ...
      <div class="detail-bottom"><a class="org-btn" href="schools.html?type=${encodeURIComponent(item.category)}&align=${encodeURIComponent(item.alignment)}">← Вернуться к Выбору</a></div>`;
    bindRenderedActions();
  }
