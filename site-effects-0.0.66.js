/* decor */
(function(){
  if(window.__KF_DECOR_V0066__)return;
  window.__KF_DECOR_V0066__=true;

  const reduceMotion=matchMedia('(prefers-reduced-motion: reduce)').matches;
  const mobile=matchMedia('(max-width: 820px)').matches;
  const file=(location.pathname.split('/').pop()||'index.html').toLowerCase();

  if(file!=='index.html'&&file!==''&&!mobile&&!reduceMotion){
    document.body.classList.add('kf-subpage-shimmer');
  }
  ...
  function sparks(x,y){
    ...
    for(let i=0;i<count;i++){
      ...
      const a=(Math.PI*2/count)*i+Math.random()*.35,r=18+Math.random()*(mobile?22:32);
  ...
  function spawn(delay=0){
    setTimeout(()=>{
      ...
      img.style.left=edge?(1+Math.random()*10)+'vw':(86+Math.random()*10)+'vw';
      img.style.setProperty('--rise',(mobile?34:26)+Math.random()*(mobile?10:12)+'s');
      ...
      img.onclick=()=>{
        ...
        spawn((mobile?9000:5000)+Math.random()*(mobile?5000:4500));
      };

      img.addEventListener('animationend',e=>{
        if(e.animationName==='kfLanternRise'){
          img.remove();
          spawn((mobile?7000:2500)+Math.random()*(mobile?6000:4500));
        }
      });
  ...
  spawn(mobile?2200:900);
  if(!mobile)spawn(5500);
})();
...
(function(){
  ...
  const TRACKS = [
    {src:new URL('kungfu-theme.mp3?v=0.0.66',base).href,title:'Тема из игры'},
    {src:new URL('kungfu-bg-32.mp3?v=0.0.66',base).href,title:'Фоновая музыка 32'},
    {src:new URL('kungfu-bg-29.mp3?v=0.0.66',base).href,title:'Фоновая музыка 29'}
  ];
